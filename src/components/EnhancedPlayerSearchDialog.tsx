    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-5xl max-h-[90vh] overflow-y-auto">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-600">Search your master database or lookup players directly from USCF.</p>
        </div>
        <div className="mb-6">
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8">
              ...
              >Search Master Database</button>
              <button
                onClick={() => setActiveTab('uscf')}
                className={`py-2 px-1 border-b-2 font-medium text-sm ${
                  activeTab === 'uscf' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                }`}
              >USCF Lookup</button>
            </nav>
          </div>
        ...
        {activeTab === 'uscf' && (
          <div>
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
              ...
              <p className="text-sm text-blue-700">Look up players directly from the USCF database. Players found here can be added to both your roster and master database.</p>
            </div>
            <div className="mb-6">
              <h3 className="text-lg font-medium mb-3">Lookup by USCF ID</h3>
              <div className="flex gap-4 items-end">
                <div className="flex-1"><label className="block text-sm font-medium mb-1">USCF ID</label><input type="text" value={uscfLookup.uscfId} onChange={(e) => setUSCFLookup(prev => ({ ...prev, uscfId: e.target.value }))} placeholder="32052572" className="w-full border rounded px-3 py-2" /></div>
                <button onClick={handleUSCFLookupById} disabled={isUSCFSearching} className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 disabled:opacity-50">{isUSCFSearching ? 'Looking up...' : 'Lookup'}</button>
              </div>
            </div>
            <div className="mb-6">
              <h3 className="text-lg font-medium mb-3">Lookup by Name</h3>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div><label className="block text-sm font-medium mb-1">First Name</label><input type="text" value={uscfLookup.firstName} onChange={(e) => setUSCFLookup(prev => ({ ...prev, firstName: e.target.value }))} placeholder="Ryan" className="w-full border rounded px-3 py-2" /></div>
                <div><label className="block text-sm font-medium mb-1">Last Name</label><input type="text" value={uscfLookup.lastName} onChange={(e) => setUSCFLookup(prev => ({ ...prev, lastName: e.target.value }))} placeholder="Moreno" className="w-full border rounded px-3 py-2" /></div>
              </div>
              <button onClick={handleUSCFLookupByName} disabled={isUSCFSearching} className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 disabled:opacity-50">{isUSCFSearching ? 'Looking up...' : 'Lookup by Name'}</button>
            </div>
            <div className="flex space-x-4 mb-6"><button onClick={handleClearUSCFSearch} className="bg-gray-600 text-white px-6 py-2 rounded hover:bg-gray-700">Clear</button></div>
            {uscfError && (<div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4"><p className="text-sm text-red-800">{uscfError}</p></div>)}
            {uscfResults.length > 0 && (
              <div className="border-t pt-4">
                <h3 className="text-lg font-semibold mb-2">USCF Results ({uscfResults.length} found)</h3>
                <div className="overflow-x-auto">
                  <table className="min-w-full border-collapse border border-gray-300">
                    <thead><tr className="bg-gray-50"><th className="border border-gray-300 px-4 py-2 text-left">Name</th><th className="border border-gray-300 px-4 py-2 text-left">USCF ID</th><th className="border border-gray-300 px-4 py-2 text-left">Regular Rating</th><th className="border border-gray-300 px-4 py-2 text-left">Quick Rating</th><th className="border border-gray-300 px-4 py-2 text-left">State</th><th className="border border-gray-300 px-4 py-2 text-left">Expires</th><th className="border border-gray-300 px-4 py-2 text-left">Action</th></tr></thead>
                    <tbody>{uscfResults.map((player) => (<tr key={player.uscf_id} className="hover:bg-gray-50">
                      <td className="border border-gray-300 px-4 py-2">{formatUSCFPlayerName(player.name)}</td><td className="border border-gray-300 px-4 py-2">{player.uscf_id}</td><td className="border border-gray-300 px-4 py-2">{player.rating_regular || 'Unrated'}</td><td className="border border-gray-300 px-4 py-2">{player.rating_quick || 'Unrated'}</td><td className="border border-gray-300 px-4 py-2">{player.state || '-'}</td><td className="border border-gray-300 px-4 py-2">{player.expiration_date || '-'}</td>
                      <td className="border border-gray-300 px-4 py-2"><button onClick={() => handleSelectPlayer(player)} disabled={excludeIds.includes(player.uscf_id)} className={`px-2 py-1 rounded text-xs ${excludeIds.includes(player.uscf_id) ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-green-500 text-white hover:bg-green-600'}`}>{excludeIds.includes(player.uscf_id) ? 'Already Added' : 'Add & Complete'}</button></td>
                    </tr>))}</tbody>
                  </table>
                </div>
              </div>
