...
const importSquareInvoicesFlow = ai.defineFlow(
  ...
  async (input) => {
    // --- START CRITICAL FIX: Just-in-time initialization ---
    const db = getDb(); 
    ...
    try {
        ...
        for (const invoice of invoicesToProcess) {
            try {
                // Pass the just-in-time clients to the processor function
...
async function processSingleInvoice(client: Client, db: FirebaseFirestore.Firestore, invoice: Invoice, batch: FirebaseFirestore.WriteBatch) {
...
function parsePlayerFromNote(note: string): { firstName: string; lastName: string; middleName?: string; uscfId?: string; isNewPlayer: boolean } | null {
    ...
    // Regex patterns to capture various name and USCF ID formats
    const patterns = [
