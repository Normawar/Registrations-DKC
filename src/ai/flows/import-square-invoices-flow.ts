  ...
  async (input) => {
    ...
    try {
      ...
      if (invoicesToProcess.length === 0) {
        return { created: 0, updated: 0, failed: 0, errors: ['No invoices found in the specified range.'] };
      }
      ...
      return { created: 0, updated: 0, failed: 1, errors: [`Square API Error: ${message}`] };
    }
  ...
  for (const item of order.lineItems) {
    ...
    for (const source of playerSources) {
      ...
      for (const note of source.split('\n')) {
        ...
        if (!selections[playerId]) selections[playerId] = { playerName: `${info.firstName} ${info.lastName}`, section: 'Unknown', baseRegistrationFee: 0 };
        if (isRegistration) selections[playerId].isRegistered = true;
        if (isUscf) selections[playerId].uscfStatus = info.isNewPlayer ? 'new' : 'renewing';
      }
