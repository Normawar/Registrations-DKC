...
function RostersPageContent() {
  ...
  // ✅ Safe data transformation (Project Preference #13)
  const safeAllPlayers = useMemo(() => {
    ...
    return allPlayers.map(player => ({
      ...
      id: player.id ? String(player.id) : crypto.randomUUID(),
    }));
  }, [allPlayers]);
  ...
  // Update schools dropdown when district filter changes
  useEffect(() => {
  ...
  // Notifications for incomplete data or duplicate emails
    ...
    <div className="p-4">
      ...
      {(notifications.incomplete.length > 0 || notifications.duplicateEmails.length > 0) && (
        <div className="bg-yellow-100 p-4 mb-4 border-l-4 border-yellow-500">
          {notifications.incomplete.length > 0 && <p>Players with incomplete required fields: {notifications.incomplete.join(", ")}</p>}
          {notifications.duplicateEmails.length > 0 && <p>Duplicate emails detected: {notifications.duplicateEmails.join("; ")}</p>}
        </div>
      )}
      ...
      <div className="flex gap-4 mb-4 flex-wrap">
        ...
        <div className="flex items-end">
          <Button variant="outline" onClick={() => { setDistrictFilter("all"); setSchoolFilter("all"); }}>
            Clear Filters
          </Button>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center gap-2 mb-4 flex-wrap">
        <div className="flex gap-2 items-center">
          ...
          <span className="text-sm text-gray-600">Showing {sortedPlayers.length} of {safeAllPlayers.length} players</span>
        </div>
      </div>
