...
function safeDateToIso(v: any): string {
  ...
  if (typeof v === 'string' && v.trim()) {
    try { const parsed = parseISO(v); if (isValid(parsed)) return parsed.toISOString(); } catch {}
  }
  try { const d = new Date(v); if (!isNaN(d.getTime())) return d.toISOString(); } catch { return ''; }
  return '';
}
...
export default function RostersPage() {
  ...
  const allPlayersRaw = master?.players ?? [];
  ...
  const safeAllPlayers = useMemo<SafePlayer[]>(() => {
    ...
    return (Array.isArray(allPlayersRaw) ? allPlayersRaw : []).map((p: any) => {
      ...
      return {
        ...
        registrationInvoice: safeString(p?.registrationInvoice),
        uscfInvoice: safeString(p?.uscfInvoice),
        pdReg: safeString(p?.pdReg),
        pdUscf: safeString(p?.pdUscf),
        ...p,
      } as SafePlayer;
    });
  }, [allPlayersRaw]);
  ...
  const scopedPlayers = useMemo(() => {
    if (!userProfile) return safeAllPlayers;
    const role = userProfile.role || (userProfile.isDistrictCoordinator ? 'district_coordinator' : 'sponsor');
    switch (role) {
      case 'organizer': return safeAllPlayers;
      case 'district_coordinator': 
      case 'district': return safeAllPlayers.filter(p => safeString(p.district) === safeString(userProfile.district));
      case 'sponsor': return safeAllPlayers.filter(p => safeString(p.school) === safeString(userProfile.school));
      case 'individual':
      case 'player':
  ...
  const sortedPlayers = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      const aVal = a[key] ?? '';
      const bVal = b[key] ?? '';
      if (['dob','uscfExpiration'].includes(key as string)) {
  ...
  const savePlayerToServer = useCallback(async (player: SafePlayer) => {
    try {
      const res = await fetch('/api/roster', { method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({player}) });
      if (!res.ok) throw new Error(await res.text() || 'Failed to update player');
      const updated = await res.json();
      setPlayers(prev => prev.map(p => p.id === player.id ? { ...p, ...updated } : p));
      return true;
    } catch (err) { console.error(err); alert(`Failed to save: ${String(err)}`); return false; }
  }, []);

  const addPlayerToServer = useCallback(async (player: Partial<SafePlayer>) => {
    try {
      const res = await fetch('/api/roster', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({player}) });
      if (!res.ok) throw new Error(await res.text() || 'Failed to create player');
      const created = await res.json();
      const sp: SafePlayer = { id: safeString(created.id||created._id), ...player, ...created } as SafePlayer;
      setPlayers(prev => [sp, ...prev]);
      return sp;
    } catch (err) { console.error(err); alert(`Failed to add: ${String(err)}`); return null; }
  }, []);

  const deletePlayerFromServer = useCallback(async (id: string) => {
    if (!confirm('Remove player?')) return false;
    try {
      const res = await fetch(`/api/roster?id=${encodeURIComponent(id)}`, { method:'DELETE' });
      if (!res.ok) throw new Error(await res.text() || 'Failed to delete player');
      setPlayers(prev => prev.filter(p => p.id !== id));
      return true;
    } catch (err) { console.error(err); alert(`Failed to delete: ${String(err)}`); return false; }
  }, []);
  ...
  const createNewPlayer = async () => {
    if (!newPlayer.firstName || !newPlayer.lastName || !newPlayer.email) { alert('First, last, email required'); return; }
    await addPlayerToServer(newPlayer); setNewPlayer({});
  };
  ...
  const exportToExcel = () => {
    const data = sortedPlayers.map(p => ({
      ...
      School: p.school,
      District: p.district,
      'USCF Exp': p.uscfExpiration ? format(parseISO(p.uscfExpiration),'MM/dd/yyyy') : '',
  ...
  const formatForDisplay = (iso?: string) => iso && isValid(parseISO(iso)) ? format(parseISO(iso),'MM/dd/yyyy') : '';
      ...
      <div className="p-4">
        ...
        {(notifications.incomplete.length>0||notifications.duplicateEmails.length>0)&&
          <div className="mb-4 bg-yellow-100 p-3 rounded border-l-4 border-yellow-500">
            {notifications.incomplete.length>0 && <div>Missing fields: {notifications.incomplete.join('; ')}</div>}
        ...
        <div className="overflow-x-auto border rounded">
          <table className="min-w-full">
            <thead className="bg-gray-100">
              <tr>
                {['Name','USCF ID','Grade','DOB','Email','Zip','School','District','Rating','USCF Exp','Actions'].map(col=>
                  <th key={col} className="p-2 text-left cursor-pointer" onClick={()=>requestSort(col==='Name'?'Name':col as keyof SafePlayer)}>
                    <div className="flex items-center gap-2">
                      <span>{col}</span>
                      {sortConfig?.key===col && <span className="text-xs">({sortConfig.direction})</span>}
                    </div>
                  </th>
                )}
              </tr>
              ...
              {sortedPlayers.map(p=>{
                ...
                return (
                  <tr key={p.id} className="border-t hover:bg-gray-50">
                    ...
                    <td className="p-2"><input className="border p-1 w-32" value={p.uscfId} onChange={e=>updateLocalField(p.id,'uscfId',e.target.value)}/></td>
                    <td className="p-2"><input className="border p-1 w-16" value={p.grade} onChange={e=>updateLocalField(p.id,'grade',e.target.value)}/></td>
                    <td className="p-2"><input className="border p-1 w-32" value={dobDisplay} onChange={e=>updateLocalField(p.id,'dob',e.target.value)}/></td>
                    <td className="p-2"><input className="border p-1 w-48" value={p.email} onChange={e=>updateLocalField(p.id,'email',e.target.value)}/></td>
                    <td className="p-2"><input className="border p-1 w-20" value={p.zip} onChange={e=>updateLocalField(p.id,'zip',e.target.value)}/></td>
                    <td className="p-2"><input className="border p-1 w-40" value={p.school} onChange={e=>updateLocalField(p.id,'school',e.target.value)}/></td>
                    <td className="p-2"><input className="border p-1 w-36" value={p.district} onChange={e=>updateLocalField(p.id,'district',e.target.value)}/></td>
                    <td className="p-2"><input className="border p-1 w-20" type="number" value={p.regularRating??''} onChange={e=>updateLocalField(p.id,'regularRating',e.target.value?Number(e.target.value):null)}/></td>
                    <td className={`p-2 ${expired?'text-red-600 font-bold':''}`}><input className="border p-1 w-36" value={expDisplay} onChange={e=>updateLocalField(p.id,'uscfExpiration',e.target.value)}/></td>
                    <td className="p-2 flex gap-2">
                      <Button onClick={()=>savePlayerToServer(p)}>Save</Button>
        ...
        <EditStudentDialog open={isSearchOpen} onClose={()=>setIsSearchOpen(false)} players={safeAllPlayers}/>
      </div>
    </AppLayout>
  );
}
