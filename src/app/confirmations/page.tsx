...
export default function ConfirmationsPage() {
  ...
  const getPlayerById = useCallback((id: string) => playersMap.get(id), [playersMap]);
  ...
  const handleByeChange = useCallback((confId: string, playerId: string, valueR1?: string, valueR2?: string) => {
    setIsUpdating(prev => ({ ...prev, [confId]: true }));
        ...
        if (conf.id === confId) {
          ...
          if (newSelections[playerId]) {
            newSelections[playerId].byes = { 
                round1: valueR1 !== undefined ? valueR1 : newSelections[playerId].byes.round1, 
                round2: valueR2 !== undefined ? valueR2 : newSelections[playerId].byes.round2 
            };
          }
  ...
  const handleAddPlayersToRegistration = async () => {
    ...
    try {
        ...
        const addedPlayersInfo = playersToAdd.map(id => getPlayerById(id)!).filter(Boolean);
