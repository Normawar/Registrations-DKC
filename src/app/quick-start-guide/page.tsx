...
export default function QuickStartGuidePage() {
  ...
  try {
    ...
    if (!existingImages.roster) {
      existingImages.roster = document.querySelector('img[alt*="roster"]') || 
                              document.querySelector('img[alt*="Roster"]') ||
                              allImages.find(img => img.src.includes('roster')) ||
                              allImages[0];
    }
    
    if (!existingImages.eventList) {
      existingImages.eventList = document.querySelector('img[alt*="event"]') || 
                                 document.querySelector('img[alt*="Event"]') ||
                                 allImages.find(img => img.src.includes('event')) ||
                                 allImages[1];
    }
    
    if (!existingImages.registrationDialog) {
      existingImages.registrationDialog = document.querySelector('img[alt*="registration"]') || 
                                          document.querySelector('img[alt*="Registration"]') ||
                                          allImages.find(img => img.src.includes('registration')) ||
                                          allImages[2];
    }
    
    if (!existingImages.invoiceDetails) {
      existingImages.invoiceDetails = document.querySelector('img[alt*="invoice"]') || 
                                      document.querySelector('img[alt*="Invoice"]') ||
                                      allImages.find(img => img.src.includes('invoice')) ||
                                      allImages[3];
    }
    
    if (!existingImages.invoicePayment) {
      existingImages.invoicePayment = document.querySelector('img[alt*="payment"]') || 
                                      document.querySelector('img[alt*="Payment"]') ||
                                      allImages.find(img => img.src.includes('payment')) ||
                                      allImages[4];
    }
    ...
    for (const [key, img] of Object.entries(existingImages)) {
      if (img) {
        try {
          console.log(`Processing image ${key}:`, { src: img.src, complete: img.complete, naturalWidth: img.naturalWidth });
          
          // Simple approach first - try direct canvas conversion
    ...
    // If no images were found at all, provide user guidance
    ...
    if (totalImagesFound === 0) {
      ...
      console.log('For better results, ensure images are visible on the page before generating PDF.');
    } else {
      console.log(`✓ Found ${totalImagesFound} images that will be included in the PDF.`);
    }
