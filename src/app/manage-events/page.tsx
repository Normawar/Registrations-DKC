...
const eventFormSchema = z.object({
  ...
  regularDeadline: z.date().optional(),
  lateDeadline: z.date().optional(),
  veryLateDeadline: z.date().optional(),
...
function ManageEventsContent() {
  ...
  const form = useForm<EventFormValues>({
    ...
    defaultValues: {
      name: '',
      date: new Date(),
      regularDeadline: undefined,
      lateDeadline: undefined,
      veryLateDeadline: undefined,
      location: '',
      rounds: 5,
      regularFee: 20,
      lateFee: 25,
    ...
    if (isDialogOpen) {
      if (editingEvent) {
        form.reset({
          ...
          regularDeadline: editingEvent.regularDeadline ? new Date(editingEvent.regularDeadline) : undefined,
          lateDeadline: editingEvent.lateDeadline ? new Date(editingEvent.lateDeadline) : undefined,
          veryLateDeadline: editingEvent.veryLateDeadline ? new Date(editingEvent.veryLateDeadline) : undefined,
  ...
  function onSubmit(values: EventFormValues) {
    const eventData = { 
      ...
      regularDeadline: values.regularDeadline?.toISOString(),
      lateDeadline: values.lateDeadline?.toISOString(),
      veryLateDeadline: values.veryLateDeadline?.toISOString(),
    };
    if (editingEvent) {
      ...
      <Dialog open={isRegistrationsOpen} onOpenChange={setIsRegistrationsOpen}>
        <DialogContent className="sm:max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription className="sr-only">View registrations for the selected event.</DialogDescription>
          ...
          <div className="space-y-4">
              {profile?.role === 'organizer' && (
                <div className="border border-amber-500 bg-amber-50 rounded-lg p-4 inline-block">
                    ...
                    <div className='flex items-center gap-2'>
                        <Button onClick={() => handleDownload(registeredPlayers, 'registered')} disabled={registeredPlayers.length === 0} size="sm" className="bg-yellow-400 text-yellow-900 hover:bg-yellow-500">
                            <Download className="mr-2 h-4 w-4" />
                            Export Registered Players ({registeredPlayers.length})
                        </Button>
                        <Button onClick={() => handleDownload(exportedPlayers, 'exported')} disabled={exportedPlayers.length === 0} size="sm" className="bg-yellow-400 text-yellow-900 hover:bg-yellow-500">
                            <Download className="mr-2 h-4 w-4" />
                          ...
                          <AccordionTrigger>
                              <div className="flex justify-between w-full pr-4 items-center">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Badge variant={invoice.invoiceStatus === 'PAID' ? 'default' : 'destructive'} className={invoice.invoiceStatus === 'PAID' ? 'bg-green-600' : ''}>{invoice.invoiceStatus}</Badge>
                                </div>
                              </div>
                          </AccordionTrigger>
                          <AccordionContent>
            ...
            <div className="py-4 space-y-4">
                <Tabs defaultValue="events">
                    ...
                    <TabsContent value="events">
                        <Textarea placeholder="Paste event data here..." className="h-64" value={pasteData} onChange={(e) => setPasteData(e.target.value)} />
                    </TabsContent>
                </Tabs>
            </div>
            <DialogFooter>
